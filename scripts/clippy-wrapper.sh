...
CLIPPY_DIR="/Users/Josh/clippy-cli"
...
# Export the CLIPPY_TOKEN from cache if available
if [ -f "$HOME/.config/clippy/token-cache.json" ]; then
    export CLIPPY_TOKEN=$(cat "$HOME/.config/clippy/token-cache.json" | jq -r '.token' 2>/dev/null)
fi
