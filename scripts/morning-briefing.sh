...
# Use clippy-wrapper to ensure token is loaded
OUTLOOK_UNREAD=$(/Users/Josh/clawd/clippy-wrapper.sh mail --unread --limit 5 2>/dev/null | grep '\[' | sed 's/\[[ 0-9]*\] • */  • /' | head -5)
...
# --- NEWS (RSS) ---
NEWS_JSON=$(python3 /Users/Josh/clawd/morning-rss-json.py 3 2>/dev/null)
NEWS=$(echo "$NEWS_JSON" | node -e '
const fs = require("fs");
try {
  const data = JSON.parse(fs.readFileSync(0, "utf8"));
  let output = "";
  data.forEach(source => {
    ...
    if (source.articles && source.articles.length > 0) {
      ...
      output += "  No new articles\n";
    }
  });
  console.log(output);
} catch(e) { console.log("News unavailable"); }
' 2>/dev/null)
